function [sensors] = diagnoser_arr_right(residues)
...
% 0 or 1 depending on which sensor is faulty.
...
sensors = 2*ones(size(residues));
for idx = 1:size(residues,2)            % Take out the dimension
    ...
    if (residue12 == 1 && residue23 == 0 && residue13 == 1)
        sensors(1,idx) = 1;
        sensors(2:3,idx) = 0;
    elseif (residue12 == 1 && residue23 == 1 && residue13 == 0)      % Take out = sign on residue23
        sensors(2,idx) = 1;
        sensors([1 3],idx) = 0;
    elseif (residue12 == 0 && residue23 == 1 && residue13 == 1)
        sensors(3,idx) = 1;
        sensors(1:2,idx) = 0;           % Change from 1:2 to 2:3
    end
end
